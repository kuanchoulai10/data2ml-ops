...
def preprocess_data(df):
    ...
    cols = ['season', 'month', 'hour', 'holiday',
    ...
    X = df_oh.drop(columns=['atemp', 'windspeed',
...
def one_hot_encoding(data, column):
    data = pd.concat([data, pd.get_dummies(
        data[column], prefix=column, drop_first=True)], axis=1)
...
def save_model_artifact(model_name, pipeline):
    ...
    model_artifact = bucket.blob(
        'bike-share-rf-regression-artifact/'+artifact_name)
